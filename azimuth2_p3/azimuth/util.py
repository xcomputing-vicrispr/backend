...
def _legacy_feature_keys(keys):
    key_set = set(keys)
    for legacy_keys in _LEGACY_FEATURE_ORDERS:
        if key_set == set(legacy_keys):
            return list(legacy_keys)
    return list(keys)


def _legacy_feature_frame(name, frame):
    if name == 'NGGX':
...
def concatenate_feature_sets(feature_sets, keys=None):
    ...
    if keys is None:
        keys = _legacy_feature_keys(list(feature_sets.keys()))
    ...
    dimsum = 0
    for set in keys:
        feature_frame = _legacy_feature_frame(set, feature_sets[set])
        inputs_set = feature_frame.values
        dim[set] = inputs_set.shape[1]
        dimsum += dim[set]
        inputs = np.hstack((inputs, inputs_set))
        feature_names.extend(feature_frame.columns.tolist())
