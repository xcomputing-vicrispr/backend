...
from azimuth._compat_sklearn import GridSearchCV, cross_val_score, cross_validation as sklearn_cv
import sklearn
        ...
        if not learn_options['adaboost_CV']:
            ...
            if learn_options["algorithm_hyperparam_search"]=="bo":
                ...
                def adaboost_scoring_bo(params):
                    ...
                    cv = sklearn_cv.KFold(y_all['Target gene'].values[train].shape[0], n_folds=20, shuffle=True)
                ...
                clf.fit(X[train], y[train].flatten())
                 ...
                 # cv = sklearn_cv.StratifiedKFold(gene_classes, n_folds=n_folds, shuffle=True)
                 cv = sklearn_cv.KFold(X[train].shape[0], n_folds=n_folds, shuffle=True)
