    ...
    if True:
        ...
        for gene in gene_names.unique():
            ...
            for j, ps in enumerate(guide_inds):
                ...
                feat.loc[feat.index[ps], "mh_score"] = mh_score
                feat.loc[feat.index[ps], "oof_score"] = oof_score
...
def local_gene_seq_features(gene_names, learn_options, X):
    ...
    for gene in gene_names.unique():
        ...
        for ps in np.where(gene_names.values==gene)[0]:
            ...
            feat.loc[feat.index[ps], "gene_left_win"] = left_win.tostring()
            feat.loc[feat.index[ps], "gene_right_win"] = right_win.tostring()
